...
impl WikiGraph {
    ...
    pub fn related(
        ...
        for (i, node) in self.nodes.iter().enumerate() {
            ...
            // same directory: +3
            if let (Some(c_dir), Some(n_dir)) =
                (center_node.id.relative.parent(), node.id.relative.parent())
            {
        ...
        let center_outgoing: HashSet<_> = self.outgoing[center_idx]
            .iter()
            .filter_map(|&e| self.edges[e].to)
            .collect();

        // common incoming links: +2 per shared
        let center_incoming: HashSet<_> = self.incoming[center_idx]
            .iter()
        ...
        for (i, _) in self.nodes.iter().enumerate() {
            ...
            let i_outgoing: HashSet<_> = self.outgoing[i]
                .iter()
                .filter_map(|&e| self.edges[e].to)
                .collect();
            ...
            if shared_out > 0 {
                add_score(
                    i,
                    2.0 * shared_out as f32,
                    &format!("{} shared outgoing links", shared_out),
                );
            }

            let i_incoming: HashSet<_> = self.incoming[i]
                .iter()
                .map(|&e| self.edges[e].from)
                .collect();
            ...
            if shared_in > 0 {
                add_score(
                    i,
                    2.0 * shared_in as f32,
                    &format!("{} shared incoming links", shared_in),
                );
            }
        }
